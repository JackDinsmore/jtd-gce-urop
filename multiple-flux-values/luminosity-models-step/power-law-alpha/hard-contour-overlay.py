...
if DRAW_EXTRA_CONTOURS:
    plt.contour(alphaVals, lMaxVals, numAboveThreshold * FERMILAB_GNFW_FLUX / DI_MAURO_FLUX, [NUM_PULSARS_ABOVE_THRESHOLD], colors=["blue"], linewidths=1, label="$N_r$ Fermilab gNFW")
    plt.contour(alphaVals, lMaxVals, numAboveThreshold * ABAZAJIAN_FLUX / DI_MAURO_FLUX, [NUM_PULSARS_ABOVE_THRESHOLD], colors=["green"], linewidths=1, label="$N_r$ Abazajian")
    plt.contour(alphaVals, lMaxVals, numAboveThreshold * AJELLO_FLUX / DI_MAURO_FLUX, [NUM_PULSARS_ABOVE_THRESHOLD], colors=["teal"], linewidths=1, label="$N_r$ Ajello")
...
# Observation
if DRAW_EXTRA_CONTOURS:
    shade(numAboveThreshold * AJELLO_FLUX / DI_MAURO_FLUX, NUM_PULSARS_ABOVE_THRESHOLD, alphaVals, lMaxVals)
else:
...
if(DRAW_EXTRA_CONTOURS):
    plt.savefig("hard-overlay-extra.png")
if(not DRAW_EXTRA_CONTOURS):
    plt.savefig("hard-overlay.png")
