...
if DRAW_EXTRA_CONTOURS:
    plt.contour(xVals, yVals, numAboveThreshold * FERMILAB_GNFW_FLUX / DI_MAURO_FLUX, [NUM_PULSARS_ABOVE_THRESHOLD], colors=["blue"], linewidths=1, label="$N_r$ Fermilab gNFW")
    plt.contour(xVals, yVals, numAboveThreshold * ABAZAJIAN_FLUX / DI_MAURO_FLUX, [NUM_PULSARS_ABOVE_THRESHOLD], colors=["green"], linewidths=1, label="$N_r$ Abazajian")
    plt.contour(xVals, yVals, numAboveThreshold * AJELLO_FLUX / DI_MAURO_FLUX, [NUM_PULSARS_ABOVE_THRESHOLD], colors=["teal"], linewidths=1, label="$N_r$ Ajello")
...
# Observation
if DRAW_EXTRA_CONTOURS:
    shade(numAboveThreshold * AJELLO_FLUX / DI_MAURO_FLUX, NUM_PULSARS_ABOVE_THRESHOLD, xVals, yVals)
else:
    shade(fracAboveThreshold, NUM_PULSARS_ABOVE_THRESHOLD, xVals, yVals)
